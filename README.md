...
Raw prices (yfinance)
↓
Realized volatility — 6 horizons: 5, 10, 21, 42, 63, 126 days
↓
Nelson-Siegel — compresses 6 values into β0 (level), β1 (slope), β2 (curvature)
↓
XGBoost walk-forward — forecasts tomorrow's betas using only past data
↓
Portfolio optimization — Methods A, B, C
↓
Backtesting — 5 strategies compared from 2018 to today
...
├── .gitignore
├── src/
│   ├── data_loader.py      ← downloads prices from yfinance
│   ├── volatility.py       ← realized volatility on 6 horizons
│   ├── nelson_siegel.py    ← Nelson-Siegel fitting, extracts β0 β1 β2
│   ├── forecasting.py      ← XGBoost and ARMA walk-forward
│   ├── portfolio.py        ← portfolio weights (Method A, B, C)
│   └── backtesting.py      ← backtesting engine and metrics
├── dashboard/
│   └── app.py              ← Streamlit dashboard
├── data/
│   ├── raw/                ← immutable raw prices
│   └── processed/          ← computed betas, forecasts, weights
├── agents/
│   ├── data-agent.md
│   └── trading-agent.md
└── outputs/
